...
const forgetpassmail = async (req, res) => {
  try {
    try {
      ...
      if (userData) {
        req.session.forgetuser = userData._id;
        // const newPassword = await bcrypt.hash(password, 10);
        const token = await otplib.authenticator.generate(secret);
        emailOtp = token;
        ...
        const message = {
          from: "vidventures.yt@gmail.com", // Sender address
          to: email, // List of recipients
          subject: "Vidventures PassWord Reset", // Subject line
          text: "Hello, this is a test email sent from Node.js using Nodemailer!", // Plain text body
          html: `<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
          <html xmlns="http://www.w3.org/1999/xhtml" xmlns:o="urn:schemas-microsoft-com:office:office" style="font-family:arial, 'helvetica neue', helvetica, sans-serif">
          <head>
...
const paypalPayment = async (req, res) => {
  ...
  try {
    ...
    const oid = orderIdCreate.generate(); //generating order id
...
const paypalSucess = async (req, res) => {
  ...
  try {
    ...
    if (user.paymentString == order.paymentString) {
      ...
      paypal.payment.execute(paymentId, exec_payment_json, function (err, payment) {
        if (err) {
      ...
      res.render("success", {totalAmount, id: orderId, cartItem, orderDetails: order, session: true });
    }
...
let amount //obj for saving paypal payment
...
const Payment = async (req, res) => {
  try {
    ...
    if (payment_method === "stripe") {
      ...
      //Converting shorts addon to payment Objects

      if (shortsqty) {
        shortsPrice = 8 * shortsqty;
        itemobj = {
          "name": "Shorts",
          "sku": '0000000001',
          "price": '8.00',
        ...
        totalPaymentPrice += shortsPrice
        orderTotal += shortsPrice
        items.push(itemobj)
      } else {
        shortsPrice = null;
      }
      ...
      //converting thumbnail to Payment Objects
      if (thumbnailqty) {
        thumbnailPrice = 5 * thumbnailqty;
        itemobj = {
          "name": "Premium thumbnail",
          "sku": '0000000002',
          "price": '5.00',
        ...
        totalPaymentPrice += thumbnailPrice

        orderTotal += thumbnailPrice
      ...
      //converting channelmagement to Payment Objects


      if (channelTotal) {
        channelManagementPrice = 150;
        itemobj = {
          "name": "Channel Management(1 Month)",
          "sku": '0000000003',
          "price": '150.00',
          "currency": "USD",
          "quantity": 1,
        }
        totalPaymentPrice += channelManagementPrice
        orderTotal += channelManagementPrice
        items.push(itemobj)
      ...
      cartItems.cart.item.forEach(item => {
        ...
        let name = item.productId.name;
        let sku = item.productId._id;
        let price = parseFloat(item.productId.discountedPrice)
        ...
        itemobj = {
          ...
          "currency": "USD",
          "quantity": qty,
          "url": `https://www.vidventuresyt.com/admin/assets/ProductImages/${image}`
        }
...
const loadSuccess = async (req, res, next) => {
  try {
    ...
    if (user.paymentString == order.paymentString) {
      ...
      const fdate = date.toLocaleDateString("en-US");
      const totaAmount = order.products.totalPrice;
    const userdta =  await userModel.findByIdAndUpdate(
        { _id: req.session.user_id },
        { $set: { paymentString: "" } }
      );
      const cartItem = await userModel.getCartItems(user._id);
      ...
      res.render("success", { id: orderId, cartItem, orderDetails: order, session: true ,totalAmount});
    }
  } catch (err) {
    console.log(err);
  }
